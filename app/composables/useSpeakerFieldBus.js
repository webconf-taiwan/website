// 首頁「一鏡到底」版（單一 canvas）專用的元件間橋接。
//
// 為什麼需要它：原版首頁裡「講者換人」是 SpeakerField.vue 自己的事 —— 那一區的
// canvas 就長在那個元件裡。單一 canvas 版本把 canvas 抽到頁面最底層（HomeField），
// 名單卻還在 HomeSpeaker 裡，兩者是兄弟元件，沒有父子關係可以傳事件。
//
// 用 provide/inject 也可以，但 useParticleStage 已經示範了「模組層級 ref = 單例」
// 的寫法，這裡沿用同一套，元件端讀起來一致。
//
// ⚠️ 模組層級狀態會跨路由存活，所以粒子那一端卸載時要把 swapImpl 清掉、
// speakerIndex 歸零，否則從首頁離開再回來會停在上次的人。
//
// ⚠️ 登記 swapImpl 的「不只一個」元件：桌機是 HomeField（整頁一張 canvas 的
// speaker 影格），窄視窗是 HomeSpeakerPortrait（觀景框裡那張小 canvas）。
...
// 由 HomeField 註冊的實作。canvas 還沒 ready（或這個頁面沒有 field）時是 null，
// 此時 selectSpeaker 只換文字，不做粒子動畫 —— 名單本身照常可用。
const swapImpl = shallowRef(null)

export function useSpeakerFieldBus () {
