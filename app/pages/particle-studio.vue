...
const ratio = computed(() => RATIOS.find(r => r.key === ratioKey.value) || RATIOS[0])
const outW = computed(() => ratio.value.w >= ratio.value.h ? longEdge.value : Math.round(longEdge.value * ratio.value.w / ratio.value.h))
const outH = computed(() => ratio.value.h >= ratio.value.w ? longEdge.value : Math.round(longEdge.value * ratio.value.h / ratio.value.w))
...
function download () {
  ...
  canvas.toBlob((blob) => {
    if (!blob) return
    const base = (sourceName.value || 'particle').replace(/\.[a-z0-9]+$/i, '')
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `particle-${base}-${outW.value}x${outH.value}.png`
    a.click()
...
onMounted(async () => {
  ...
  speakerPortraits.value = (mod.default?.items || [])
    .filter(s => s.portrait)
    .map(s => ({ name: s.name, portrait: s.portrait }))
})
      ...
      <div class="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div>
          <div class="mb-3 flex flex-wrap gap-2">
            <button
              v-for="p in speakerPortraits"
              :key="p.portrait"
              type="button"
              class="h-14 w-14 overflow-hidden rounded-lg border-2 transition"
              :class="sourceUrl === p.portrait ? 'border-white' : 'border-neutral-700 hover:border-neutral-400'"
              :title="p.name"
              @click="loadFromUrl(p.portrait, p.name)"
            >
              <img :src="p.portrait" :alt="p.name" class="h-full w-full object-cover">
            </button>
          ...
          <div
            class="relative mx-auto overflow-hidden rounded-2xl bg-[#0a0a0c]"
            :style="{ aspectRatio: `${outW} / ${outH}`, maxHeight: '70vh', width: `min(100%, ${outW}px)` }"
          >
        ...
        <div class="space-y-6 rounded-2xl border border-neutral-800 p-5">
          <div>
            <h2 class="mb-3 font-mono text-xs tracking-widest text-neutral-500">
              取樣（改了會重新取樣）
            </h2>
            ...
            <div class="mb-4">
              ...
              <input v-model.number="samples" type="range" min="2000" max="40000" step="500" class="w-full accent-white">
            </div>
            ...
            <div>
              ...
              <input v-model.number="sampleEdge" type="range" min="240" max="960" step="40" class="w-full accent-white">
            </div>
          </div>
